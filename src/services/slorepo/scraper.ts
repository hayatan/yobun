...
import { cleanNumber, calculateMaxMY, calculateMaxMDia } from '../../util/slorepo';
import { Hole, Machine, SlotData, ProcessedSlotData } from '../../types/slorepo';

export default async function scrapeSlotDataByMachine(
    date: string,
    holeCode: string,
    interval: number = 3000
): Promise<ProcessedSlotData[]> {
...
// データ処理関数
function processSlotData(allData: SlotData[]): ProcessedSlotData[] {
...
async function getMachines(page: Page): Promise<Machine[]> {
    return await page.evaluate(() => {
        const machineLinks = Array.from(document.querySelectorAll('a[href^="kishu/?kishu="]'));
        return machineLinks.map(link => ({
            name: link.textContent?.trim() || '',
            encodedName: (link as HTMLAnchorElement).href.split('kishu/?kishu=')[1]
        }));
    });
}

async function scrapeMachineHtmlData(
    page: Page,
    url: string,
    date: string,
    holeName: string,
