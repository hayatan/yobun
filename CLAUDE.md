...
2. **コード変更時は dev-flow スキルに従う** — 実装・修正・リファクタ・テスト・設計すべて。フローの詳細は `/dev-flow` に定義。必須ステップはスキップ禁止。任意ステップ（例: Step 2.5, Step 7）は dev-flow の条件に従い省略可。
3. **codex MCP は `gpt-5.3-codex` + `xhigh` で呼ぶ** — `model: gpt-5.3-codex`, `config.reasoning_effort: xhigh` を必ず指定。省略・変更禁止。ユーザーが明示的に別の値を指定した場合のみ上書き可。
4. **規模が大きいタスクではエージェントとタスク管理を活用する** — 複数ファイルにまたがる変更や、複数の独立した作業が発生する場合は、Task ツールでエージェントを spawn して並列に進める。計画と進捗は `.tasks/plans/` に Markdown で記録する（テンプレート: `.tasks/templates/node.md`、ルール: `.tasks/AGENTS.md`）。**注意: `.tasks/` は開発用のAI作業メモであり、nekonote の機能とは一切無関係。混同禁止。**
