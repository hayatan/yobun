...
-- 
-- スプレッドシートから参照するクエリ
-- bqconfシートでパラメータを定義し、データコネクタ経由で取得
--
-- パラメータ:
--   @DATE_FROM     - 集計開始日 (YYYYMMDD形式)
--   @DATE_TO       - 集計終了日 (YYYYMMDD形式)
--   @HOLE          - 店舗名
--   @SPECIAL_CHECK - 特日フィルタを有効にするか (TRUE/FALSE)
--   @DAY_OF_WEEK   - 曜日フィルタ (月,火,水,木,金,土,日)
--   @DAY_TYPE      - 日タイプフィルタ (平日,週末,祝日)
--   @LAST_DIGIT    - 日付末尾フィルタ (0-9)
--   @MONTH_EQ_DAY  - 月=日フィルタ (TRUE/FALSE)
--   @DOUBLE_DIGIT  - ゾロ目フィルタ (TRUE/FALSE)
--
-- 出力カラム:
--   machine_number - 台番
--   machine        - 機種名
...
WITH params AS (
  SELECT
    ...
    CAST(@HOLE AS STRING) AS hole,
    CAST(@SPECIAL_CHECK AS BOOL) AS special_check,
    CAST(@DAY_OF_WEEK AS STRING) AS day_of_week,
    CAST(@DAY_TYPE AS STRING) AS day_type,
    CAST(@LAST_DIGIT AS STRING) AS last_digit,
    CAST(@MONTH_EQ_DAY AS BOOL) AS month_eq_day,
    CAST(@DOUBLE_DIGIT AS BOOL) AS double_digit
),
...
ranked_data AS (
  SELECT
    ...
    d.win,
    -- 日の数値
    EXTRACT(DAY FROM PARSE_DATE('%Y-%m-%d', d.date)) AS day,
    -- 曜日（日本語表記）
    CASE 
    ...
    END AS day_of_week,
    -- 平日・週末・祝日（週末優先）- LEFT JOINで祝日判定
    CASE 
      WHEN EXTRACT(DAYOFWEEK FROM PARSE_DATE('%Y-%m-%d', d.date)) IN (1, 7) THEN '週末'
      WHEN h.holiday_date IS NOT NULL THEN '祝日'
      ELSE '平日'
    END AS day_type,
    -- 日付の末尾の数字
    CAST(RIGHT(d.date, 1) AS INT) AS day_last_digit,
    -- 月と日が同じか
    CASE 
      WHEN EXTRACT(MONTH FROM PARSE_DATE('%Y-%m-%d', d.date)) = EXTRACT(DAY FROM PARSE_DATE('%Y-%m-%d', d.date)) THEN TRUE
      ELSE FALSE
    END AS is_month_equal_day,
    -- 日がゾロ目（11, 22）
    CASE 
      WHEN EXTRACT(DAY FROM PARSE_DATE('%Y-%m-%d', d.date)) IN (11, 22) THEN TRUE
      ELSE FALSE
...
aggregated_data AS (
  ...
  WHERE data.row_num = 1
    ...
    AND (
      ...
      -- 曜日判定
      OR CAST(p.day_of_week AS STRING) LIKE CONCAT('%', CAST(data.day_of_week AS STRING), '%') 
      -- 平日・週末・祝日判定
      OR CAST(p.day_type AS STRING) LIKE CONCAT('%', CAST(data.day_type AS STRING), '%') 
      -- 日付の末尾の数字判定
      OR CAST(p.last_digit AS STRING) LIKE CONCAT('%', CAST(data.day_last_digit AS STRING), '%') 
      -- 月と日が同じか
      OR (data.is_month_equal_day AND p.month_eq_day)
      -- ゾロ目
      OR (data.is_day_double_digit AND p.double_digit)
    ) 
  GROUP BY
    data.hole,
    data.machine,
...
-- ============================================================================
-- 6. 最新データ判定（ウィンドウ関数で相関サブクエリを回避）
-- ============================================================================
aggregated_data_with_max AS (
  SELECT
    *,
    MAX(date_to) OVER (PARTITION BY machine_number) AS max_date_to
